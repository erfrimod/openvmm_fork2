...
mod tests {
    ...
    async fn test_default_packet_send(
        ...
        let expected_num_recvd_packets = 1;
        let (sent_data, tx_segments) =
    ...
    fn build_tx_segments(
        ...
        let sent_data = (0..packet_len).map(|v| (0 + v) as u8).collect::<Vec<u8>>();
    ...
    async fn test_endpoint(
        ...
        while rx_packets_n == 0 {
            ...
            // GDMA Errors generate a TryReturn error, so we ignore errors here.
            tx_done_n += queues[0].tx_poll(&mut tx_done[tx_done_n..]).unwrap_or(0);
            if expected_num_recvd_packets == 0 {
                break;
            }
        }
        assert_eq!(rx_packets_n, expected_num_recvd_packets);

        if expected_num_recvd_packets == 0 {
            // If no packets were received, we are done.
    ...
    async fn test_valid_packet(driver: DefaultDriver) {
        ...
        assert_eq!(stats.tx_packets, 1, "Expected tx_packets to increase");
        assert_eq!(stats.rx_packets, 1, "Expected rx_packets to increase");
        assert_eq!(stats.tx_errors, 0, "Expected tx_errors to remain the same");
        assert_eq!(stats.rx_errors, 0, "Expected rx_errors to remain the same");
    }

    #[async_test]
    async fn test_rx_error_handling(driver: DefaultDriver) {
        let expected_num_recvd_packets = 0;
        let num_segments = 2;
        // bnic test emulator treats 1234 as a signal to return CQE_RX_ERR_DISABLED_QUEUE
        let packet_len = 1234;
        let metadata = net_backend::TxMetadata {
            id: TxId(1),
            segment_count: num_segments,
        ...
        assert_eq!(stats.rx_errors, 1, "Expected rx_errors to increase");
        assert_eq!(stats.rx_packets, 0, "Expected rx_packets to stay the same");
    }
    ...
    async fn test_tx_error_handling(driver: DefaultDriver) {
        ...
        assert_eq!(stats.tx_errors, 1, "Expected tx_errors to increase");
        assert_eq!(stats.tx_packets, 0, "Expected tx_packets to stay the same");
    }
