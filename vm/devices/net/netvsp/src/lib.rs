...
// race of netvsc and VF register_netdevice"). Non-Linux guests are not
// affected.
#[cfg(not(test))]
...
/// Windows, FreeBSD, and other guests do not need the delay.
...
impl Nic {
    ...
    fn insert_worker(
        ...
        // Update the coordinator state VF device delay for the primary channel.
        if channel_idx == 0 {
            self.coordinator.task_mut().vf_device_delay = vf_device_delay;
        }
        Ok(())
    }
}
...
impl InspectTaskMut<Coordinator> for CoordinatorState {
    fn inspect_mut(
        ...
        if let Some(coordinator) = coordinator {
            if let Some(worker) = coordinator.workers[0].state() {
                resp.sensitivity_field(
                    "vf_device_delay_ms",
                    SensitivityLevel::Safe,
                    worker.channel.vf_device_delay.as_millis() as u64,
                )
                .sensitivity_field(
                    "link_delay_duration_ms",
                    SensitivityLevel::Safe,
                    worker.channel.link_delay_duration.as_millis() as u64,
                );
            }
