...
enum InteropAction {
    ...
    OidSetWellFormedConfigParameter {
        ...
        name_index: u8,
        /// Fuzz-selected parameter type (STRING / INTEGER / HEX_INTEGER /
        /// MULTI_STRING / BINARY / unknown).
        param_type: rndisprot::NdisParameterType,
        /// Raw value payload (interpretation depends on `param_type`).
        value_bytes: Vec<u8>,
    },
    ...
    /// Stop the channel, save state, restore it, and restart.  Exercises
    /// the coordinator stop/start cycle and save/restore serialization
    /// round-trip from a live state.
    SaveAndRestore,
    /// Stop and restart the channel without save/restore.  Exercises the
    /// coordinator stop → start transition with existing state.
    StopAndRestart,
}
...
async fn execute_next_action(
    ...
    match action {
        ...
        InteropAction::ControlSendRawInBand {
            ...
            // via `InteropTeardownAction` / `TeardownAction`.
        ...
        InteropAction::InjectTxRestart {
            ...
            if let Some(mode) = tx_error_mode {
                ...
                fuzz_helpers::drain_queue_returning_rx_bufs(queue).await;
            }
        }
        // ==== Common ====
        ...
        InteropAction::SaveAndRestore => {
            // Stop → save → restore → start round-trip.
            handle.channel.stop().await;
            if let Ok(Some(blob)) = handle.channel.save().await {
                let _ = handle.channel.restore(blob).await;
            }
            handle.channel.start();
            fuzz_helpers::yield_to_executor(10).await;
        }
        InteropAction::StopAndRestart => {
            handle.channel.stop().await;
            handle.channel.start();
            fuzz_helpers::yield_to_executor(10).await;
        }
...
async fn run_interop_loop(
    ...
    let result = run_interop_loop_inner(
        fuzz_input,
        &handle,
        setup,
        rx_send,
...
async fn run_interop_loop_inner(
    fuzz_input: &[u8],
    handle: &NicSetupHandle,
    setup: fuzz_helpers::nic_setup::FuzzNicSetup,
    rx_send: &mesh::Sender<(Vec<u8>, FuzzRxMetadata)>,
    ...
    while !fuzzer_input.is_empty() {
        ...
        if actions_since_drain >= 4 {
            fuzz_helpers::drain_queue_returning_rx_bufs(&mut queue).await;
