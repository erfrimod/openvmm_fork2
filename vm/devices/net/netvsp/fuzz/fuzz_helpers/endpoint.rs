...
impl FuzzableQueue {
    ...
    fn materialize_injected_packets(&mut self, pool: &mut dyn net_backend::BufferAccess) {
...
impl BackendQueue for FuzzableQueue {
    fn poll_ready(
        ...
        self.materialize_injected_packets(pool);
...
impl Endpoint for FuzzEndpoint {
    ...
    async fn get_queues(
        ...
        queues.extend(config.into_iter().enumerate().map(|(idx, _config)| {
