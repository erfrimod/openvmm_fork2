...
// Each fuzz binary re-declares `mod fuzz_helpers;`, so items used only by a
// subset of binaries appear dead in the others.
#![allow(dead_code)]
...
//! [`drain_queue_async`], [`run_fuzz_loop_with_config`]
...
pub async fn send_tx_rndis_completion(
    ...
    send_completion_packet(queue, transaction_id, &[&payload]).await
}
...
/// Drain pending packets from the queue, sending back
/// `MESSAGE1_TYPE_SEND_RNDIS_PACKET_COMPLETE` for every `Data` packet
/// received (which are host→guest RX deliveries).  This returns RX buffer
/// IDs to the NIC worker so the endpoint can reuse them for subsequent
/// RX packets, preventing RxId exhaustion.
pub async fn drain_queue_returning_rx_bufs(queue: &mut Queue<GpadlRingMem>) {
    const MAX_DRAIN_ROUNDS: usize = 16;
    const QUIESCENT_EMPTY_ROUNDS: usize = 2;

    let completion_payload = nvsp_payload(
        protocol::MESSAGE1_TYPE_SEND_RNDIS_PACKET_COMPLETE,
    ...
    let mut rx_tids: Vec<u64> = Vec::new();
    let mut consecutive_empty_rounds = 0;
    for _ in 0..MAX_DRAIN_ROUNDS {
        yield_to_executor(1).await;
        let mut drained_any = false;
        // Phase 1: read all pending packets, collecting transaction IDs
        // from Data packets (host→guest RX deliveries).
        rx_tids.clear();
        loop {
            let (mut reader, _) = queue.split();
            match reader.try_read() {
                Ok(packet) => {
                    drained_any = true;
                    if let IncomingPacket::Data(data) = &*packet {
                        if let Some(tid) = data.transaction_id() {
                            rx_tids.push(tid);
                        }
                    }
                }
        ...
        // Phase 2: send completions for all collected RX transaction IDs
        // to return RX buffer IDs to the worker.
        for &tid in &rx_tids {
            let (_, mut writer) = queue.split();
            let _ = writer.try_write(&OutgoingPacket {
                transaction_id: tid,
                packet_type: OutgoingPacketType::Completion,
...
/// Actions that tear down the NIC or halt RNDIS processing. These should be
/// chosen by fuzz input at the start of the test, then executed after the
/// main fuzz loop is exhausted, rather than being interleaved with normal
/// actions (which would prematurely terminate the worker).
#[derive(Arbitrary, Debug)]
pub enum TeardownAction {
    /// Do nothing — the NIC stays running and gets cleaned up normally.
    None,
    /// Revoke the receive buffer (terminates the NIC worker).
    RevokeReceiveBuffer,
    /// Revoke the send buffer (terminates the NIC worker).
    RevokeSendBuffer,
    /// Send an RNDIS RESET message (terminates the worker via
    /// `WorkerError::RndisMessageTypeNotImplemented`).
    SendRndisReset { reserved: u32 },
    /// Send an RNDIS HALT message (terminates the worker).
    SendRndisHalt,
    /// Inject a fatal TX error (terminates the worker via `WorkerError::Endpoint`).
    InjectTxFatal {
        ppi_bytes: Vec<u8>,
        frame_data: Vec<u8>,
    },
...
/// Execute a teardown action after the main fuzz loop.
///
/// `tx_error_mode` is only needed for `InjectTxFatal`; pass `&None` if
/// TX error injection is not configured.
pub async fn execute_teardown(
    ...
    layout: &PageLayout,
    tid: &mut u64,
    tx_error_mode: &Option<std::sync::Arc<std::sync::atomic::AtomicU8>>,
) -> Result<(), anyhow::Error> {
    match teardown {
        TeardownAction::None => {}
        TeardownAction::RevokeReceiveBuffer => {
            let msg = protocol::Message1RevokeReceiveBuffer { id: 0 };
            send_inband_nvsp(
                queue,
                tid,
        ...
        TeardownAction::RevokeSendBuffer => {
            let msg = protocol::Message1RevokeSendBuffer { id: 0 };
            send_inband_nvsp(
                queue,
                tid,
        ...
        TeardownAction::SendRndisReset { reserved } => {
            let rndis_bytes =
                build_rndis_message(rndisprot::MESSAGE_TYPE_RESET_MSG, &reserved.to_le_bytes());
            let _ = send_rndis_control(queue, mem, &rndis_bytes, layout, tid).await;
            drain_queue_async(queue).await;
        }
        TeardownAction::InjectTxFatal {
            ppi_bytes,
            frame_data,
        } => {
