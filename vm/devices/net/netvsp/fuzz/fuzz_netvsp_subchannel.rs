...
async fn execute_next_action(
    ...
    match action {
        ...
        SubchannelAction::OpenPendingSubchannels => {
            ...
            // Use the async drain so the NIC worker actually gets CPU time
            // between the preceding `RequestSubchannels` send and
            // `open_pending()`.
        ...
        SubchannelAction::DisableRss => {
            ...
            let rndis_buf =
                build_rss_oid_set(0, &[0; 4], 1, rndisprot::NDIS_RSS_PARAM_FLAG_DISABLE_RSS);
            send_rndis_gpadirect(
                queue,
                mem,
                &rndis_buf,
        ...
        SubchannelAction::ChangeRssIndirection { seed } => {
            ...
            if !open_queues.is_empty() {
                ...
                // Use Toeplitz + IPv4 hash type to enable RSS.
                let hash_info: u32 =
                    rndisprot::NDIS_HASH_FUNCTION_TOEPLITZ | rndisprot::NDIS_HASH_IPV4;
